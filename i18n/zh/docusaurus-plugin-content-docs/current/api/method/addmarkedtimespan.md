...
description: "标记日期，并且在某些设置下，可以阻止这些日期（允许对限制的边界应用自定义样式）"
---
...
请注意，调用此方法后，标记（阻止）不会立即生效。您需要调用 [updateView](api/method/updateview.md) 来应用更改。
...
~~~
  ...
  <td> 指定时间段类型。设置为 'dhx_time_block' 则应用阻止。其他值仅标记时间段，不阻止。</td>
  </tr>
  <tr>
  <td colspan="2" >
~~~js
...
~~~
  ...
  <td> 指示是否应反转 'zones' 中设置的时间段（默认为false）</td>
  </tr>
  <tr>
  <td colspan="2" >
~~~js
...
~~~
  ...
  <td> 限制阻止仅针对特定视图中的特定条目。<br>日期仅在指定视图中被阻止。</td>
  </tr>
  <tr>
  <td colspan="2" >
~~~js
...
<tr><td markdown='1'>
需要调用 [updateView](api/method/updateview.md) 来渲染时间段的DIV 
</td><td markdown='1'>
自动绘制时间段的DIV 
</td></tr>
<tr><td markdown='1'>
