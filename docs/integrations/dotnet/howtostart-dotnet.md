---
title: "dhtmlxScheduler with ASP.NET MVC"
sidebar_label: "ASP.NET MVC"
---

# dhtmlxScheduler with ASP.NET MVC

	This tutorial will give you step-by-step instructions on creating Scheduler with [ASP.NET](https://dotnet.microsoft.com/en-us/apps/aspnet) and REST API on the server side. 
...
the [Entity Framework](https://learn.microsoft.com/en-us/aspnet/entity-framework). We will build our application with the help of the Visual Studio IDE.
...
- added dhtmlx scheduler js/css sources using [CDN links](guides/installation.md#cdn)
- created scheduler on the page

And also we told the scheduler that it's going to work with the RESTful API on a backend and use ["/api/scheduler/"](guides/server-integration.md#technique) to get all data during loading and as a default route:
...
:::note
In case Scheduler doesn't render events on a page, check the [Troubleshooting Backend Integration Issues](guides/troubleshooting.md) article.
:::
...
[Exception filters](https://learn.microsoft.com/en-us/previous-versions/aspnet) can be used for capturing exceptions in CRUD handlers and returning a client response that can be
[recognized](guides/server-integration.md#error-handling) by the client-side scheduler.
...
Scheduler doesn't provide any means of preventing an application from various threats, such as SQL injections, XSS and CSRF attacks. The responsibility for keeping an application safe is on the developers who implement the backend. Read the details in the [corresponding article](guides/app-security.md).
...
In case you've completed the above steps to implement Scheduler integration with ASP.NET MVC, but Scheduler doesn't render events on a page, have a look at the [Troubleshooting Backend Integration Issues](guides/troubleshooting.md) article. 
