---
title: "dhtmlxScheduler with dhtmlxConnector"
...
To load data from an inline object, use the [parse](api/method/parse.md) method. 
...
[mapped to the lightbox](guides/lightbox-editors-manipulations.md#mapping-properties-of-an-event-object-to-the-lightbox-sections).
...
So, to provide correct data conversion, you should change the default scheduler format. It can be achieved by specifying the [date_format](api/config/date_format.md) configuration option.
...
To load data from a database, use the [load](api/method/load.md) method where specify the backend url as a parameter. 
You can implement the backend by yourself using our [guides](integrations/howtostart-guides.md), but for this tutorial we'll cover the [PHP connector library](https://docs.dhtmlx.com/connector__php__index.html) as the quickest way.
...
:::note
You can read about recurring events in the [Recurring Events](guides/recurring-events.md#server-side-logic) article.
:::
...
To 'force' the scheduler save data in the database, use [dataProcessor](guides/server-integration.md#technique).
