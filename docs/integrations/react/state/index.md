...
description: "How to bind React Scheduler to React state or a state manager, handle user edits, and choose between React-managed and Scheduler-managed data models."
---
...
If your app performs large operations (bulk edits, frequent updates, big datasets), pay attention to:

- using `data.batchSave` to reduce update overhead,
- the **Scheduler-managed data** model when React does not need to reflect every change immediately.
