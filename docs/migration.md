...
The [all_timed](api/config/all_timed.md) plugin is enabled by default, allowing the display of overnight events. The previous behavior can be restored via the configuration:
...
The new engine for [Recurring events](guides/recurring-events.md) will be used when the `recurring` plugin is activated:
...
The new [undo_deleted](api/config/undo_deleted.md) config is enabled by default. You can disable it via the config, if this behavior doesn't suit your needs:
...
The following properties are deprecated and included into the [map_settings](api/config/map_settings.md) configuration object:
...
The following templates of the Map view are deprecated and are replaced by [map_info_content](api/template/map_info_content.md):
...
The new [map_view_provider](api/config/map_view_provider.md) property can be specified both separately and inside the [map_settings](api/config/map_settings.md) configuration object like this:
...
The map properties listed below are used outside the [map_settings](api/config/map_settings.md) configuration object:
...
Check the available variables on the [Skins Customization](guides/custom-skins.md) page.
...
Or the [setSkin](api/method/setskin.md) method:
...
The default values of [details_on_create](api/config/details_on_create.md) and [details_on_dblclick](api/config/details_on_dblclick.md) properties have changed from `false` to `true`.
...
Tooltips have a new API that provides easy attachment of tooltip to custom elements. See more details in the related article: [Tooltips](guides/tooltips.md).
...
To remove this padding, use [day_column_padding](api/config/day_column_padding.md):
...
The **Bluebird** library has been excluded from the Scheduler bundle. [Promise](api/method/promise.md) now uses the native Promise implementation.
...
The default handler for the [swipe gesture](guides/touch-support.md#touch-gestures-in-the-scheduler) has been disabled by default.

You can re-enable it using the [scheduler.config.touch_swipe_dates](api/config/touch_swipe_dates.md) config, as follows:
...
[Month View](views/month.md) has been changed from **content-box** to **border-box** in all skins.
...
[drag_event_body](api/config/drag_event_body.md) and set it to *false* (by default the property is enabled).
...
- after v5.2 it is set by the [date_format](api/config/date_format.md) property and its value is "%Y-%m-%d %H:%i"
...
If you want to restore previous behavior and get dates in the format defined by the user, use the [parse_exact_format](api/config/parse_exact_format.md) configuration option:

~~~js
scheduler.config.parse_exact_format = true;
~~~

### "vertical" setting of the [Multiselect](guides/multiselect.md#properties) control can't take string value
...
If you use them, you'll either need to migrate to another [skin](guides/skins.md) or keep using a corresponding CSS file from an older version.
...
Since v.4.3 the extensions [Week Agenda View](views/weekagenda.md), [Grid View](views/grid.md), [Timeline View](views/timeline.md), [Units View](views/units.md), [Multisection Events](api/config/multisection.md) 
...
For more details check [Skins](guides/skins.md).

- [multi_day](api/config/multi_day.md) is enabled by default, and if you want to revert such a behavior, add:
...
+ Scheduler now accepts [JSON generated by dhtmlxConnector](guides/server-integration.md#json-mode).
