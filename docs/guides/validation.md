...
Generally, to validate the data entered by the user, use the events provided by the [dhtmlxScheduler API](api/overview/events_overview.md) and catch the input data to process them in accordance with their correctness:

## Client-side validation

The following events are mostly important and commonly used for data validation:

- [onEventSave](api/event/oneventsave.md) - fires when the user clicks on the 'Save' button in the lightbox
- [onBeforeEventCreated](api/event/onbeforeeventcreated.md) - fires before a new event is added to the Scheduler
- [onBeforeEventChanged](api/event/onbeforeeventchanged.md) - fires  before an event is updated


The simplest validation can be achieved with the help of the [onEventSave](api/event/oneventsave.md) event. The event is invoked when the user clicks the 'Save' button on the form. 
...
To prove this and catch all changes made in the Scheduler (editing, creating, deleting etc.), use the [dataProcessor](guides/server-integration.md) object or, to be precise, one of its events - 
