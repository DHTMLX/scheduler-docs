---
title: "Recurring Events (up to v7.1)"
sidebar_label: "Recurring Events (up to v7.1)"
---
...
- [include_end_by](api/config/include_end_by.md) - defines whether the date specified in the 'End by' field should be exclusive or inclusive
- [recurring_overflow_instances](api/config/recurring_overflow_instances.md) - defines the behavior of the recurrences that transfer to the next month
- [repeat_precise](api/config/repeat_precise.md) - prevents including past days to events with the 'weekly' recurrence
- [occurrence_timestamp_in_utc](api/config/occurrence_timestamp_in_utc.md) - allows working with recurring events independently of time zones
...
So, your backend should return recurring fields along with regular event fields, for example:
...
:::note
You can find complete code examples [here](integrations/howtostart-guides.md)
:::


Implement this logic in your backend controller or service to keep recurring series and exceptions consistent.
...
1. Define the markup of a custom form and place it somewhere on the page (you can start by copying the default template, which can be found at <b>'schedulersourceslocalerecurring'</b>):
~~~html
...
You can find the default HTML structure of the lightbox's recurring block for different languages at the <b>'schedulersourceslocalerecurring'</b> directory.


For example,for the English locale you need to see the <b>'schedulersourceslocalerecurringrepeat_template_en.htm'</b> file.
...
Possible values are <b>'no'</b>, <b>'date_of_end'</b>, <b>'occurences_count'</b>.
