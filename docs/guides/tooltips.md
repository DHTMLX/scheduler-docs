...
*If you use dhtmlxScheduler 6.0 or earlier, see details [here](guides/tooltips-legacy.md).*
...
There are two corresponding methods in the [tooltip API](#tooltip-api) for this purpose:

- the [**scheduler.ext.tooltips.tooltipFor()**](#methods) method 

Note, the [scheduler.ext.tooltips.tooltipFor()](#methods) method must be called after the Scheduler initialization is complete. For instance, you can specify the method inside the [onSchedulerReady](api/event/onschedulerready.md) event handler like this:
...
A tooltip added in this way will follow the mouse pointer and use the *[tooltip_offset_x](api/config/tooltip_offset_x.md)*, *[tooltip_offset_y](api/config/tooltip_offset_y.md)*, *[tooltip_timeout](api/config/tooltip_timeout.md)*, and
[tooltip_hide_timeout](api/config/tooltip_hide_timeout.md) settings.

- the [**scheduler.ext.tooltips.attach()**](#methods) method 
...
- Remove the built-in tooltip handler from tasks with the [**scheduler.ext.tooltips.detach**](#methods) method:
...
- Add the desired tooltip behavior via the [**scheduler.ext.tooltips.attach()**](#methods) method. In the example below the tooltip is shown only above the table:
...
To specify the time period (in milliseconds) before a tooltip for a task will appear, use the [tooltip_timeout](api/config/tooltip_timeout.md) property:
...
To define how long (in milliseconds) a tooltip will be shown after the user moves the cursor to another position, use the [tooltip_hide_timeout](api/config/tooltip_hide_timeout.md) property:
...
- [tooltip_offset_y](api/config/tooltip_offset_y.md) - sets the vertical offset of the tooltip position
