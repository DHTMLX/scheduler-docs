...
sidebar_label: "Mini Calendar (Date Picker)"
---
...
~~~
3. Provide the logic that will initialize (the [renderCalendar](api/method/rendercalendar.md) method) and destroy (the [destroyCalendar](api/method/destroycalendar.md) method) the mini calendar:
~~~js
...
If you use the [markup approach for initialization](guides/initialization.md#initializing-scheduler-via-markup), you can do this, as in:
...
If you use the [header config](guides/initialization.md#initializing-scheduler-via-header-config), you'll need to add [a custom element](api/config/header.md) there:
...
If you use a separate element to display the active date of the scheduler, you'll need to capture the [onViewChange](api/event/onviewchange.md) event of the scheduler and update the date label from there:
...
Note, that we don't use this handler in our code sample, since we rely on the built-in date header of the scheduler which is updated automatically. You need to use such code only if you [hide the default date header](guides/scheduler-markup.md#hiding-the-header-of-scheduler), or if you need to display the active date in multiple places.
...
~~~
3. Call the [renderCalendar](api/method/rendercalendar.md) method to render the mini calendar on the page:
~~~js
...
### Templates
To customize the format of dates presented in the mini calendar (date picker), you can use a number of templates listed in the article [Mini Calendar Templates](guides/mini-calendar-templates.md). 
...
To assign a custom CSS class to a day, you can use the [markCalendar](api/method/markcalendar.md) method:
