...
Recurring events are a common feature in event calendar applications, allowing users to create events that repeat at specified intervals. Starting from v7.1 the Scheduler uses the [RFC-5545](https://datatracker.ietf.org/doc/html/rfc5545) based format for recurring events.
...
By default, Scheduler doesn't support recurring events. To provide such support, you need to enable the `recurring` extension on the page:
...
**Related sample** [Recurring events](https://docs.dhtmlx.com/scheduler/samples/03_extensions/01_recurring_events.html)
...
The repeated event scheduled for `2027-06-10 09:00:00` will be replaced with the `Special Team Meeting` record, and the event scheduled for `2027-06-17 09:00:00` will be skipped.
...
It means that all event handlers or custom properties that have been attached to DOM elements of your form from the code won't be applied to the form in the lightbox.
If you want to attach an event handler, you need either to specify it as an inline HTML attribute, or attach a handler to the form when it's shown in the lightbox with `addEventListener()`.
