...
1. <b>Activate the 'multiselect' extension on the page:</b>
~~~js
scheduler.plugins({
    multiselect: true
});
...
~~~
3. <b>Set the label for the section:</b>
~~~js
scheduler.locale.labels.section_userselect = "Participants";
~~~
...
The following properties are mostly important and commonly set for the 'multiselect' control (see the full list [here](api/config/lightbox.md)):
...
<table class="webixdoc_links">
  ...
  <td>(<i>string</i>) specifies the delimeter that will be used to separate values of the multiselect. If this property isn't set, the [section_delimiter](api/config/section_delimiter.md) config will be applied</td>
  </tr>
  </tbody>
</table>
...
To retrieve checkbox values from the server, you need to use the [serverList](api/method/serverlist.md) method:
...
where the **api/data** is a [server-side script](guides/server-integration.md), retrieving events loaded to the scheduler, and a collection of multiselect buttons values
as shown here [Examples of Data Formats](guides/data-formats.md#json-with-collections):
...
:::note
Note, you can use the [updateCollection](api/method/updatecollection.md) method to update the list of retrieving options
:::
