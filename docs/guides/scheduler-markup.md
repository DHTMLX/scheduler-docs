...
To set the position manually (e.g. in the markup), set the [fix_tab_position](api/config/fix_tab_position.md) parameter to *false* to disable the default behaviour and set the coordinates with CSS properties:
...
:::note
Even if the tab is removed, the related view is still accessible programmatically with the [setCurrentView](api/method/setcurrentview.md) and [updateView](api/method/updateview.md) methods.
:::
