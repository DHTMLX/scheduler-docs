...
description: "How to back up and keep using the paid DHTMLX @dhx Scheduler npm packages after your subscription ends."
---

# Using Packages After Your Subscription Expires

The paid Scheduler packages - the commercial JavaScript library (`@dhx/scheduler`) and the React wrapper (`@dhx/react-scheduler`) - are hosted on the private registry at `https://npm.dhtmlx.com`. Access to that registry is tied to your subscription: once it expires, you can no longer install or reinstall these packages from npm.
...
:::note
This applies only to the paid `@dhx` packages. The public `dhtmlx-scheduler` (Standard) and `@dhtmlx/trial-*` (evaluation) packages live on the public npm registry and are not affected.
:::
...
cd dhtmlx-backup
npm init -y
npm install <your-dhtmlx-packages>   # e.g. npm install @dhx/scheduler @dhx/react-scheduler
~~~
...
This generates a `.tgz` file in the current directory (for example `dhx-scheduler-9.0.10.tgz`). Copy the generated `.tgz` files to a safe location - for example a `./lib/` folder in your project.
...
~~~json
"dependencies": {
  "@dhx/scheduler": "file:./lib/dhx-scheduler-9.0.10.tgz"
}
~~~
