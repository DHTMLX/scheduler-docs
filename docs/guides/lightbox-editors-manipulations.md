---
title: "Manipulations with Lightbox"
sidebar_label: "Manipulations with Lightbox"
---
...
To get/set the value of the section's control, use the [formSection](api/method/formsection.md) object as in:
...
It is possible to open the existing lightbox on a single click. For this purpose, make use of the [onClick](api/event/onclick.md) event and [showLightbox](api/method/showlightbox.md) method:
...
- Make sure your data source returns events in a [supported format](guides/data-formats.md)
...
Note, that all properties your data source returns will be added to event objects and will available for the [client-side API](guides/event-object-operations.md).
...
Only exceptions are the [time](guides/time.md) and the [recurring](guides/recurring-events.md#recurring-lightbox) controls that are always mapped to a fixed properties (**event.start_date/event.end_date** and **event.rec_type/event.event_length/event.event_pid** respectively).
...
the [event_duration](api/config/event_duration.md) and [auto_end_date](api/config/auto_end_date.md) properties:
...
To set the default value directly for new events, use the [onEventCreated](api/event/oneventcreated.md) event:
...
The detailed information about the read-only mode see in chapter [Read-only Mode](guides/readonly.md).
...
To add the 'full-day event' option to the lightbox, set the [full_day](api/config/full_day.md) option to *true*.
...
Once, the [full_day](api/config/full_day.md) option is enabled, the **Full Day** checkbox 
...
To set the desired type, use the [wide_form](api/config/wide_form.md) property:
...
You can change the lightbox sections dynamically via [the resetLightbox()](api/method/resetlightbox.md) method. For instance:
