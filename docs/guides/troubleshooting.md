...
Does the response look similar to the [expected data format](guides/data-formats.md).
...
- `start_date`, `end_date` - make sure date formats match the format you specify in the [date_format](api/config/date_format.md) config of Scheduler

[Read details about the data properties](guides/loading-data.md#data-properties).
...
Check what is stored in your database, probably the issue is there. If date formats don't match, either change the [date_format](api/config/date_format.md) config, 
...
e.g. year 1970 or year 2038. This will obviously indicate a problem with the [date_format](api/config/date_format.md) format.

:::note
If you use the [recurring extension](guides/recurring-events.md), [scheduler.getEvents()](api/method/getevents.md) won't return any data, unless you limit the output by the `from`/`to` date parameters. 
Try removing the recurring events extension from the page for this check.
:::

### Missing properties for Units/Timeline

Or, if you use the [Units](views/units.md) or [Timeline](views/timeline.md) views, events may be not displayed, if they are not mapped to any section of the unit/timeline.

In order to check it, you can use `skip_incorrect:false` for the [Units view](views/units.md):
...
Or, `show_unassigned: true` for the [Timeline view](api/method/createtimelineview.md):
...
If you don't see any obvious issues there, you may want to check your code, probably events are [hidden by filter](guides/filtering.md).
