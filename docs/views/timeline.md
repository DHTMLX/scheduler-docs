...
1.<b>Activate the Timeline extension on the page:</b>
  <ul>
...
~~~
4. <b>Call the [createTimelineView](api/method/createtimelineview.md) method:</b>
~~~js
...
In spite of a big number of parameters used in the [createTimelineView](api/method/createtimelineview.md) method, it's quite simple. 
...
they are defined dynamically from the timeline constructor and will be overwritten by the [createTimelineView](api/method/createtimelineview.md) call. 
...
the [timeline_scale_label](api/template/timelinename_scale_label.md) template.


In order to specify multiple columns, use the **columns** property of the [createTimelineView](api/method/createtimelineview.md) method:
...
:::note
The functionality described in this section works only for the Timeline with a [horizontal scroll enabled](#horizontal-scroll).
:::
...
All defined timeline objects are stored in the [scheduler.matrix](api/other/matrix.md) object.
...
If you need to dynamically update a list of units in the Timeline view, check the following methods: [serverList](api/method/serverlist.md) and [updateCollection](api/method/updatecollection.md) ones.
...
* [y_property](api/method/createtimelineview.md) - (*string*) the name of a data property that will be used to assign events to certain sections.
...
The needed mode is set by the [render](api/method/createtimelineview.md) parameter:
...
1. Time scale must cover 1 day exactly. If your configuration specifies a shorter or longer period, timeline will be rendered incorrectly:
~~~js
scheduler.createTimelineView({
    name:"timeline", 
    render:"days", 
...
~~~
2. The mode doesn't support [Blocking and Marking functionality](guides/limits.md)

The format of the Y-Axis labels is specified by the [scale_label](api/template/timelinename_scale_label.md) template:
~~~js
...
~~~
3. To change the start day of the displayable interval, use the <b>scheduler.date.TIMELINE_start</b> function:
~~~js
...
- the [first_hour](api/method/createtimelineview.md) and [last_hour](api/method/createtimelineview.md) parameters:
...
- the **ignore_(viewName)** property, where the **viewName** part is the value of the *name* option of the object passed to the [createTimelineView](api/method/createtimelineview.md) method.
...
The **ignore_\{viewName}\** method is described in more detail in the section [](guides/custom-scales.md).
...
To set values for the Y-Axis in the 'Bar' and 'Cell' modes, use the [y_unit](api/method/createtimelineview.md) parameter:
...
To be correctly processed, [y_unit](api/method/createtimelineview.md) items must have 2 mandatory properties:
...
To set values for the 'Tree' timeline, use the same [y_unit](api/method/createtimelineview.md) parameter that is used in the 'Bar' and 'Cell' modes, but with a wider set of properties:
...
The [y_unit](api/method/createtimelineview.md) items have:
...
To set values for the Y-Axis in the 'Days' mode, use the [days](api/method/createtimelineview.md) parameter:
...
*where [serverList](api/method/serverlist.md)  returns a list of options with the name 'sections'*.

- On the server side 

The data response for the [load](api/method/load.md) method should contain a collection with the server list name specified in JSON [of the following format](guides/data-formats.md#json-with-collections).
...
The data response should have the [JSON format](guides/data-formats.md#json-with-collections) that includes
the "collections" hash of necessary collections to be referenced through the [serverList](api/method/serverlist.md) method.
...
:::note
To use the [addSection](api/method/addsection.md) and [deleteSection](api/method/deletesection.md) methods, the 'Tree' timeline must be currently opened in the scheduler.
:::
...
You can add the second scale by using the [second_scale](api/method/createtimelineview.md) parameter: 
...
To make an event occupy the entire cell width, no matter how long this event lasts, use the [round_position](api/method/createtimelineview.md) parameter:
...
of the [sort](api/method/createtimelineview.md) parameter:
...
[Sorting by the text length](Sorting by the text length)
~~~js
scheduler.createTimelineView({
    name:   "timeline",
    render:"bar",
...
In order to add a horizontal scrollbar into the timeline, you should use the **scrollable** property of the [createTimelineView](api/method/createtimelineview.md) method.
...
There is a possibility to adjust sensibility and speed of autoscroll via the related properties of the **autoscroll** object. Use it inside of the the [createTimelineView](api/method/createtimelineview.md) method:
...
By default the header of the column with sections is empty. You can add a label into this header via the [locale](api/other/locale.md) object in the following way:

~~~js
scheduler.locale.labels.<timelineName>_scale_header = "Label";
~~~

where \<timelineName\> is the name of the timeline created with the [createTimelineView](api/method/createtimelineview.md) method. For example:
...
To enable this functionality for a particular timeline, make use of the **cell_template** property while creating the timeline with the [createTimelineView](api/method/createtimelineview.md) method.
...
By default, sections and events get their sizes from the **dy** and **event_dy** settings of the [createTimelineView](api/method/createtimelineview.md) method.
