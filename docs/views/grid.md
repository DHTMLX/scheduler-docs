...
1. <b>Activate the "grid view" extension on the page:</b>
~~~js
scheduler.plugins({
    grid_view: true
});
...
~~~
4. <b>Call the [createGridView](api/method/creategridview.md) method:</b> 
~~~js
...
To activate navigation with buttons ![navigation_buttons](/img/navigation_buttons.png) in the grid, enable the [paging](api/method/creategridview.md) property:
...
Once the navigation is enabled, when the user clicks on ![navigation_buttons](/img/navigation_buttons.png) buttons - 
...
When you click on the header, the grid starts to display a special control indicating, which column the table is currently sorted by and the direction of this sorting (ascending or descending).
...
To enable sorting and assign the appropriate sorting type to a column, use the [sort](api/method/creategridview.md) property.
...
If you want to apply a custom sorting behavior, define the related logic in a function and set this function as the value of parameter [sort](api/method/creategridview.md).
...
Data templates are set in the related column by the property [template](api/method/creategridview.md). 
