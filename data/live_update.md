...
{{note This article refers to implementation of the Live Updates mode for DHTMLX Scheduler v7.2. See the details on the previous versions [here](live_update_legacy.md). }}

## Principle

DHTMLX Scheduler provides a `RemoteEvents` helper to synchronize changes among multiple users in real-time.

### Key Workflow

- The `RemoteEvents` client opens a WebSocket connection when the Scheduler is initialized.
- User changes (create, edit, or delete events) are sent to the server via `DataProcessor` using the REST API.
...
After recieving a response, `RemoteEvents` establishes the websocket connection with the provided endpoint.
...
The second part of this module is `remoteUpdates` helper that is responsible for parsing messages received via websocket and applying appropriate changes to Scheduler.
...
`RemoteEvents.on` method expects the object argument which can specify handlers for one or multiple entities:
...
When initialized that way, the `remoteEvents` object will send websocket a subscription message formatted in the following way:

~~~json
{"action":"subscribe","name":"calendars"}
~~~

And the handler will be called whenever messages directed to the specified entity is received:
