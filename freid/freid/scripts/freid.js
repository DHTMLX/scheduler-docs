var Freid = {
	...
	_get_by_type:function(col, pull, order, name, check, fossil, ignore_key){
		var ignore = ignore_key?(scheduler.debug_freid_ignore[ignore_key]||scheduler.debug_freid_ignore["*"]):null;
		for (var key in col){
	...
	_is_code_template:function(code, pull, order, objname, obj, method_name, ignore){
		var re = /(scheduler|this)\.templates\.([a-zA-Z_]+)/g;
		var match;

		var match = code.match(re);
		if (match){
			for (var i=0; i<match.length; i++){
				var name = match[i].replace("scheduler.templates.","").replace("this.templates","");
	...
	_is_code_property:function(code, pull, order, objname, obj, method_name, ignore){
		var re = /scheduler\.config\.([a-zA-Z_]+)/g;
		var match;

		var match = code.match(re);
		if (match){
			for (var i=0; i<match.length; i++){
				var name = match[i].replace("scheduler.config.","");
				if (ignore && ignore[name]) return false;
	...
	_get_views_list:function(){
		...
		this.vOrder = ["scheduler"];
		this.vPull = { scheduler:obj };
	...
	_force_from_docs:function(data){
		var arts = scheduler.debug_freid_artf[data.fullname];
		var confs = scheduler.debug_freid_config[data.fullname];
		var events = scheduler.debug_freid_event[data.fullname];
		var templates = scheduler.debug_freid_template[data.fullname];
